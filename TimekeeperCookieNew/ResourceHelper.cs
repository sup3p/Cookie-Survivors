    ...
    public static class ResourceHelper
    {
        public static Texture2D LoadTexture(string resourceName)
        {
            ...
            using (Stream stream = assembly.GetManifestResourceStream(resourcePath))
            {
                ...
                //This block taken from Takacomic's DirectersAssistant
                unsafe
                {
                    var ptr = UnityEngine.Object.MarshalledUnityObject.MarshalNotNull(texture);
                    fixed (byte* bytesPtr = imageData)
                    {
                        var managedSpanWrapper = new ManagedSpanWrapper(bytesPtr, imageData.Length);
